...
*
*/
/* Edited by Tushar Patel to implement active class animation and fixed a bug to implement proper class activation. */
;(function($, window, document) {
    ...
    $.fn.stickyNavbar = function(prop) {
        ...
        sections.attr('tabindex', -1); // TODO: remove this or outline : 0
        ...
        return this.each(function() {
            ...
            menuItems.click(function(e) {
                ...
                // Fixing incorrect scrolling. Not counting things like padding.
                var toScroll = $self.hasClass(options.unstickyModeClass) ? sectionOffsets[currentHref] - 2 * thisHeight - 72  + 'px' : sectionOffsets[currentHref] - thisHeight - 72 + 'px';
            ...
            var mainFunc = function() {
                ...
                menuItems.stop().removeClass(options.activeClass);

                // add activeClass to the div that is passing the top of the window
                sections.each(function() {
                    var top = $(this).offset().top - thisHeight - 100,
                    bottom = $(this).outerHeight(true) + top;

                    if ((windowPosition >= top) && (windowPosition <= bottom)) {
                        if (options.selector === 'a') {
                            $self.find('li a[href~="#' + this.id + '"]').stop().addClass(options.activeClass);
                        } else {
                            $self.find('li a[href~="#' + this.id + '"]').parent().stop().addClass(options.activeClass);
                        }
                    }
                });
                ...
                //if (typeof lastSection !== 'undefined') {
                    ...
                    if (win.scrollTop() + windowHeight >= $(document).height() && windowPosition <= lastSectionBottom) {
                        ...
                        menuItems.stop().removeClass(options.activeClass).last().stop().addClass(options.activeClass);
                    }
